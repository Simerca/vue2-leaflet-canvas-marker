/*!
 * @simerca/vue2-leaflet-marker-canvas v1.0.1
 * (c) 
 * Released under the ISC License.
 */
'use strict';

var utils_js = require('vue2-leaflet/dist/utils/utils.js');
var L$1 = require('leaflet');
var Layer = require('vue2-leaflet/dist/mixins/Layer.js');
var Options = require('vue2-leaflet/dist/mixins/Options.js');
var vue = require('vue');
...
var Options__default = /*#__PURE__*/_interopDefaultLegacy(Options);
...
var CanvasIconLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  addMarkers: function addMarkers(markers) {
    ...
    markers.forEach(function (marker) {
      ...
      var s = self._addMarker(marker, latlng, isDisplaying); //Only add to Point Lookup if we are on map
  ...
  addMarker: function addMarker(marker) {
    ...
    var dat = self._addMarker(marker, latlng, isDisplaying); //Only add to Point Lookup if we are on map
  ...
  addLayer: function addLayer(layer) {
    if (layer.options.pane == 'markerPane' && layer.options.icon) this.addMarker(layer);else console.error('Layer isn\'t a marker');
  },
  ...
  removeMarker: function removeMarker(marker, redraw) {
    var self = this; //If we are removed point
  ...
  onAdd: function onAdd(map) {
    ...
    if (this.options.pane) this.getPane().appendChild(this._canvas);else map._panes.overlayPane.appendChild(this._canvas);
  ...
  onRemove: function onRemove(map) {
    if (this.options.pane) this.getPane().removeChild(this._canvas);else map.getPanes().overlayPane.removeChild(this._canvas);
  ...
  _addMarker: function _addMarker(marker, latlng, isDisplaying) {
    var self = this; //Needed for pop-up & tooltip to work.

    marker._map = self._map; //_markers contains Points of markers currently displaying on map

    if (!self._markers) self._markers = new rbush(); //_latlngMarkers contains Lat\Long coordinates of all markers in layer.
    ...
    self._latlngMarkers.dirty++;
    self._latlngMarkers.total++; //Only draw if we are on map

    if (isDisplaying === true) self._drawMarker(marker, pointPos);
    return ret;
  },
  _drawMarker: function _drawMarker(marker, pointPos) {
    var self = this;
    ...
    if (marker.canvas_img) {
      ...
      if (self._imageLookup[iconUrl]) {
        ...
        var i = new Image();
        i.src = iconUrl;
        marker.canvas_img = i; //Image,isLoaded,marker\pointPos ref
  ...
  _drawImage: function _drawImage(marker, pointPos) {
  ...
  _redraw: function _redraw(clear) {
    ...
    var tmp = []; //If we are 10% individual inserts\removals, reconstruct lookup for efficiency
    ...
    var mapBounds = self._map.getBounds(); //Only re-draw what we are showing on the map.
    ...
    self._latlngMarkers.search(mapBoxCoords).forEach(function (e) {
      ...
      tmp.push(newCoords); //Redraw points
  ...
  addOnClickListener: function addOnClickListener(listener) {
    this._onClickListeners.push(listener);
  },
  addOnHoverListener: function addOnHoverListener(listener) {
    this._onHoverListeners.push(listener);
  },
  _executeListeners: function _executeListeners(event) {
...
var script = {
  name: "LMarker",
  mixins: [Layer__default['default'], Options__default['default']],
  props: {
    pane: {
      type: String,
      "default": "markerPane"
  ...
  mounted: function mounted() {
    ...
    this.doJob(); // ciLayer.addMarker(this.mapObject)
    // console.log(ciLayer);
  ...
  methods: {
    doJob: function doJob() {
      utils_js.optionsMerger(Object.assign({}, this.layerOptions, {
        icon: this.icon,
        zIndexOffset: this.zIndexOffset,
        draggable: this.draggable,
        opacity: this.opacity
      }), this); // Marker definition

      this.mapObject = L__default['default'].canvasIconLayer({}).addTo(this.$parent.$el.__vue__.mapObject);
      var markers = [];
      ...
      this.debouncedLatLngSync = utils_js.debounce(this.latLngSync, 100);
      this.mapObject.on("move", this.debouncedLatLngSync);
      utils_js.propsBinder(this, this.mapObject, this.$options.props);
      this.parentContainer = utils_js.findRealParent(this.$parent);
    },
...
var index = {
  install: function install(Vue, options) {
    ...
    Vue.component("l-canvas-marker", script);
  }
};
